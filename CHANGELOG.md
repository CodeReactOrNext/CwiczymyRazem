...
### Added
- Metronome click sounds: Settings → Tablature (and the in-session tab settings) now offer six synthesised clicks — classic beep, wood block, digital blip, sticks, closed hi-hat and cowbell. Tapping one previews it, the choice is saved on the device, and both the desktop and mobile metronome engines pick it up on the next click.
...
- Pedalboard: extra Fame/h ("Full chain") when the board carries a pedal of every available effect type at once — stacks with the wiring-order bonus and shows up in Signal path / Signal order on the Rig.

### Changed
- Trzy duże pedale-efekty typu "Lab" (Stereo Fuzz Lab, Stereo Delay Lab, Overdrive Pro) dają teraz wyższy Item Level, więc mocniej podbijają Fame rate riga niż zwykły pedal tej samej rzadkości.
