...
function GrantPremiumForm({
  ...
  onGrant: (userId: string, premiumUntil?: string) => Promise<void>;
}) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    await onGrant(userId.trim(), premiumUntil || undefined);
    setUserId("");
    setPremiumUntil("");
    setLoading(false);
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-3 sm:flex-row">
        ...
        <div className="flex items-end">
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-cyan-600 px-5 py-2.5 text-sm font-bold text-black transition hover:bg-cyan-500 disabled:cursor-not-allowed disabled:opacity-40 sm:w-auto"
          >
            <Crown size={14} />
      ...
      <p className="mt-2 text-[10px] text-zinc-700">
        Leave expiry empty for permanent (forever) premium access.
      </p>
    </form>
  );
}
...
const AdminPremiumPage = () => {
  ...
  const active   = users.filter((u) => ["forever", "active"].includes(getStatus(u.premiumUntil))).length;
  ...
  const statCards = [
    ...
    { label: "Active",          value: active,   icon: Sparkles,     color: "emerald"},
  ...
  return (
    <AdminLayout onLogout={handleLogout}>
      <div className="space-y-8 p-8 animate-in fade-in duration-700">
        ...
        {/* Grant form */}
        <GrantPremiumForm onGrant={grantPremium} />
