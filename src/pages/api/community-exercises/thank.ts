...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const thanksRef = exerciseRef.collection("thanks").doc(userId) as DocumentReference;
    ...
    await firestore.runTransaction(async (t: Transaction) => {
      ...
      t.update(authorRef, { "statistics.fame": FieldValue.increment(THANKS_FAME_REWARD) });
    });

    return res.status(200).json({ success: true, fameAwarded: THANKS_FAME_REWARD });
  } catch (error: any) {
    switch (error.message) {
      case "EXERCISE_NOT_FOUND":
        return res.status(404).json({ error: "Exercise not found" });
      case "OWN_EXERCISE":
        return res.status(400).json({ error: "You cannot thank your own exercise" });
      case "ALREADY_THANKED":
        return res.status(409).json({ error: "You already thanked this exercise" });
