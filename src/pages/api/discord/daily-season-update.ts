...
async function getTopSeasonalPlayers(seasonId: string, limit: number = 5) {
  ...
  const q = query(seasonalUsersRef, orderBy("points", "desc"), limit(limit));
    ...
    const leaderboardMessage = {
      embeds: [{
        ...
        description: topPlayers
          .map((player, index) => 
            `${index + 1}. **${player.displayName}** - ${player.points} punktów`)
          .join("\n"),
        color: 0xf1c40f
      }]
    };

    await sendDiscordMessage(leaderboardMessage);
