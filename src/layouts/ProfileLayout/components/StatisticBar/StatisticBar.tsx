...
const StatisticBar = ({ title, value, percent }: StatisticBarProps) => {
  ...
  return (
    <div className=' m-1 flex flex-col  items-center font-openSans sm:text-xl'>
      ...
      <p className='my-2 w-full border-2 border-main-opposed-400/80 bg-main-opposed p-1 px-3  text-center text-[0.6rem] tracking-wider radius-default xs:text-xs sm:px-10 sm:text-sm md:px-9 lg:px-6 xl:px-8 '>
        {value}
      </p>
      <p className='text-center text-[0.6rem]  text-gray-200 xs:text-xs sm:text-sm'>
        {title}
      </p>
    </div>
  );
