...
export const StaticTablature: React.FC<StaticTablatureProps> = ({
  ...
  const output = STRING_NAMES.map(
    (name, idx) => `${name}|${tabLines[idx]}`,
  ).join("\n");

  return (
    <div className={`overflow-x-auto ${className}`}>
      <pre className='font-mono whitespace-pre-wrap break-words rounded-lg bg-white p-4 text-xs leading-5 text-zinc-800'>
        {output}
      </pre>
    </div>
  );
