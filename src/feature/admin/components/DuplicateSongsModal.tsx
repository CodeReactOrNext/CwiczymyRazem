...
interface DuplicateSongsModalProps {
  ...
  onMerge: (sourceId: string, targetId: string) => Promise<any>;
  scannedCount?: number;
}

export const DuplicateSongsModal = ({ isOpen, onClose, duplicates, onDelete, onMerge, scannedCount }: DuplicateSongsModalProps) => {
  const [mergingGroupId, setMergingGroupId] = useState<string | null>(null);
  const [deletingId, setDeletingId] = useState<string | null>(null);

  if (!isOpen) return null;

  const isProcessing = !!mergingGroupId || !!deletingId;
  ...
  const handleMergeGroup = async (targetSong: Song, groupSongs: Song[]) => {
    const sources = groupSongs.filter(s => s.id !== targetSong.id);
    if (sources.length === 0) return;

    if (!confirm(`This will move all users from ${sources.length} duplicate(s) to "${targetSong.title}" and delete the duplicates. Proceed?`)) return;

    setMergingGroupId(targetSong.id);
    try {
      for (const source of sources) {
        await onMerge(source.id, targetSong.id);
      }
    } finally {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 backdrop-blur-md bg-black/60 animate-in fade-in duration-300">
      <div className="w-full max-w-4xl max-h-[85vh] flex flex-col rounded-3xl border border-white/10 bg-zinc-900 font-sans shadow-2xl shadow-black/50">
        ...
        <div className="flex items-center justify-between p-8 border-b border-white/5 shrink-0">
          ...
          <button onClick={onClose} disabled={isProcessing} className="p-2 hover:bg-white/5 rounded-xl transition-colors group disabled:opacity-20">
              ...
              {duplicates.map((group) => (
                <div key={group.key} className="bg-black/30 rounded-2xl border border-white/5 overflow-hidden">
                  <div className="px-6 py-3 bg-white/[0.02] border-b border-white/5 flex items-center justify-between">
                     ...
                     <span className="text-[10px] font-bold text-zinc-600 uppercase tracking-widest">{group.songs.length} versions found</span>
                  </div>
                  <div className="divide-y divide-white/5">
                    {group.songs.map((song) => (
                      <div key={song.id} className="p-4 flex items-center justify-between hover:bg-white/[0.01] transition-colors">
                        <div className="flex items-center gap-4">
                           {song.coverUrl ? (
                             <img src={song.coverUrl} className="h-10 w-10 rounded-lg object-cover bg-black shadow-lg" />
                           ) : (
                             <div className="h-10 w-10 rounded-lg bg-zinc-800 flex items-center justify-center text-zinc-600 text-[10px] font-bold border border-white/5 italic">N/A</div>
                           )}
                           <div>
                              ...
                              <div className="flex gap-4 mt-1">
                                <span className="text-[10px] font-bold text-zinc-600">ID: {song.id.slice(0, 8)}</span>
                                <span className="text-[10px] font-bold text-cyan-500 bg-cyan-500/10 px-1.5 rounded border border-cyan-500/10">Rating: {song.avgDifficulty}</span>
                                <span className="text-[10px] font-bold text-purple-500 bg-purple-500/10 px-1.5 rounded flex items-center gap-1 border border-purple-500/10">
                        ...
                        <div className="flex items-center gap-2">
                          <Button 
                            onClick={() => handleMergeGroup(song, group.songs)}
                            disabled={isProcessing}
                            variant="ghost"
                            size="sm"
                            className="h-9 hover:bg-cyan-500/10 hover:text-cyan-400 text-zinc-500 font-black uppercase tracking-wider text-[10px]"
                          >
                            {mergingGroupId === song.id ? (
