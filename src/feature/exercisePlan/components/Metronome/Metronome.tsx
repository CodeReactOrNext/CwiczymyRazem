...
export const Metronome = ({
  ...
  return (
    <Card className="relative overflow-hidden rounded-lg border-0 bg-card/80 p-3 shadow-xl backdrop-blur-sm">
      ...
      <div className="flex flex-col items-center justify-center py-2 mt-1">
        {locked ? (
          <div className="flex flex-col items-center gap-2">
            <div className="flex items-center gap-2">
              <Lock size={14} className="text-amber-400" />
              <span className={cn("text-4xl font-black tabular-nums select-none", tempoColor)}>{bpm}</span>
            </div>
          </div>
        ) : isEditingBpm ? (
          <input
            ...
            className="w-20 bg-zinc-900/50 rounded-lg px-2 py-1 text-center text-4xl font-black text-cyan-400 outline-none ring-2 ring-cyan-500 tabular-nums"
          />
        ) : (
          <button
            onClick={handleBpmClick}
            className={cn("text-4xl font-black tabular-nums transition-transform hover:scale-105 active:scale-95 cursor-pointer select-none underline decoration-dashed underline-offset-4 decoration-white/20 hover:decoration-white/50", tempoColor)}
            title="Click to edit BPM"
          >
            {bpm}
          </button>
        )}
        <span className="mt-2 text-[10px] font-bold uppercase tracking-[0.2em] text-zinc-500 select-none">
      ...
      <div className="mt-1 flex flex-col gap-2">
        <div className="flex items-center gap-3">
          <Button
            variant="ghost"
            size="icon"
            className="h-7 w-7 shrink-0 rounded-lg bg-zinc-800/40 hover:bg-zinc-700/80 text-zinc-100 hover:text-white transition-colors"
          ...
          <div className="flex-1 flex flex-col gap-1">
            <Slider
              ...
              className={cn("py-1.5", locked ? "cursor-not-allowed opacity-50" : "cursor-pointer", getSliderTrackColor(bpm))}
            />
