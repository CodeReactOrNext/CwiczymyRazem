...
export function generateSingleStringScaleExercise(config: SingleStringScaleConfig): Exercise {
  ...
  return {
    ...
    title: `${rootNote} ${scaleDef.name} – ${stringName}`,
    description: `Play ${rootNote} ${scaleDef.name} linearly along ${stringName}. Work through all scale notes on a single string.`,
    difficulty: "easy",
    category: "theory",
    timeInMinutes: 5,
    instructions: [
      `Play ${rootNote} ${scaleDef.name} along ${stringName} (string ${stringNum}).`,
      `Move along the fretboard – do not change strings.`,
      `Play up and down: from the lowest to the highest note and back.`,
      `Use alternate picking (down-up-down-up).`,
      `Start slowly, focus on clean tone.`,
    ],
    tips: [
      `Playing on a single string helps you understand scale intervals and note positions.`,
      `${scaleDef.description}`,
      `Visualize where the scale notes are – this builds fretboard awareness.`,
    ],
