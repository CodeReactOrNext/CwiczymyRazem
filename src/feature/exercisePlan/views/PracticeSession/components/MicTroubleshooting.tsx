...
export const MicTroubleshooting = ({ compact = false, className }: MicTroubleshootingProps) => {
    ...
    <>
      {compact ? (
        <RippleButton
          ...
          className={cn(
            "flex items-center justify-center h-8 w-8 rounded-lg transition-all active:scale-90 bg-zinc-800 text-zinc-400 hover:text-white",
            className
          )}
        >
        ...
        <RippleButton
          onClick={() => setIsOpen(true)}
          className={cn(
            "flex items-center gap-1.5 text-[11px] font-semibold tracking-wide text-zinc-500 hover:text-zinc-200 transition-colors",
            className
          )}
        >
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="sm:max-w-lg !flex flex-col max-h-[100dvh] sm:max-h-[85vh] z-[99999999]">
          <DialogHeader className="shrink-0">
            ...
            <DialogTitle className="text-center text-xl">Microphone Troubleshooting</DialogTitle>
            <DialogDescription className="text-center text-zinc-400 leading-relaxed pt-1">
              If the app can&apos;t hear your guitar or detects the wrong notes, work through
              these steps. Most issues come from browser permissions or another app using the mic.
            </DialogDescription>
          </DialogHeader>

          <ul className="mt-2 flex flex-1 min-h-0 flex-col gap-3 overflow-y-auto pr-1">
            {TIPS.map((tip, idx) => {
              const Icon = tip.icon;
              return (
                <li key={idx} className="flex gap-3 rounded-lg bg-zinc-900/60 p-3">
          ...
          <DialogFooter className="shrink-0 flex flex-col gap-2 sm:flex-col sm:gap-2 pt-4">
            <Button
            ...
            <p className="text-center text-[10px] text-zinc-500">
              Clears this app&apos;s saved calibration and mic preferences, then reloads the page.
            </p>
            <Button variant="ghost" onClick={() => setIsOpen(false)} className="w-full text-zinc-500 hover:text-zinc-300">
              Close
            </Button>
          </DialogFooter>
        </DialogContent>
