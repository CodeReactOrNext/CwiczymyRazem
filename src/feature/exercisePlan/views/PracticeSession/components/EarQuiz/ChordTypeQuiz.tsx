...
export function ChordTypeQuiz({ question, isAnswered, isCorrect, onAnswer, onNext }: ChordTypeQuizProps) {
    ...
    <>
      <div className='flex flex-col items-center gap-4 py-2'>
        ...
        <p className='text-center text-xs text-zinc-500'>
          {isAnswered ? `Root: ${question.rootName}` : "The root moves every round — listen to the colour, not the pitch"}
        </p>
      </div>

      <div className={cn("grid gap-3", question.options.length > 4 ? "grid-cols-2 sm:grid-cols-4" : "grid-cols-2")}>
