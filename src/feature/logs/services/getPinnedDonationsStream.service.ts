...
/**
 * The pinned donations, streamed on their own instead of being picked out of the feed page.
 *
 ...
 * be pinned to the top of it. This query is bounded by the 24-hour pin window rather than by a row
 * count, so the donation stays up for as long as it's supposed to.
 *
 * Capped at the number of donations the feed pins anyway: past the fourth nothing more is shown,
 * so there's no reason to stream them.
 *
 ...
 */
export const firebaseGetPinnedDonationsStream = (
  callback: (logs: AnyFirebaseLog[]) => void,
  now: Date = new Date(),
) => {
  const pinnedDonations = query(
    collection(db, "logs"),
  ...
  return onSnapshot(
    ...
    (error) => {
      ...
      console.error("Pinned donations listener failed:", error);
      callback([]);
    },
  );
};
