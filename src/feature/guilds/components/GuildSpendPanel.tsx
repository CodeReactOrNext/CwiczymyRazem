...
interface GuildSpendPanelProps {
  currency: SpendCurrency;
  /** What the panel is for, drawn beside the title. */
  icon?: LucideIcon;
  title: ReactNode;
...
export const GuildSpendPanel = ({
  ...
  return (
    <section
      className={cn(
        "relative flex h-full flex-col overflow-hidden rounded-lg p-5 sm:p-6",
        look.surface,
        className,
      )}>
        ...
        <div className='flex flex-wrap items-start justify-between gap-x-6 gap-y-3'>
          <div className='flex min-w-0 items-start gap-3'>
            {Icon && (
              <span
                className={cn(
                  "flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-zinc-950/40",
                  look.text,
                )}>
                <Icon size={19} />
              </span>
            ...
            <div className='min-w-0 space-y-1'>
              ...
              {blurb && (
                <div className='max-w-xl text-sm leading-relaxed text-zinc-400'>
                  {blurb}
                </div>
              )}
            </div>
...
export const PledgeButton = ({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "flex items-center gap-1.5 rounded-lg px-3.5 py-2.5 text-sm font-bold tabular-nums transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring disabled:pointer-events-none disabled:opacity-40",
