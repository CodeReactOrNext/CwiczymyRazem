...
export const SkillCategoryCard = ({
  ...
  const getCategoryColors = () => {
    switch (category) {
      case "technique": return "text-red-400 bg-red-400/10";
      case "theory": return "text-blue-400 bg-blue-400/10";
      case "hearing": return "text-emerald-400 bg-emerald-400/10";
  ...
  return (
    <motion.div
      ...
      className="relative rounded-lg p-0 backdrop-blur-sm transition-background sm:bg-zinc-900/40 sm:p-6 sm:hover:bg-zinc-900/60"
    >
      <div className="flex flex-col h-full gap-5">
        ...
        <div className="flex-1">
          {unlockedSkills.length > 0 ? (
            <div className="flex flex-col gap-2">
              {unlockedSkills.map((skill) => {
                ...
                return (
                  <div
                    key={skill.id}
                    className="flex w-full items-center justify-between gap-2 px-3 py-2 rounded-lg bg-zinc-800/40 transition-background hover:bg-zinc-800/70 group"
                  >
                    ...
                    <span className="text-[10px] font-bold text-zinc-400 tabular-nums bg-white/5 px-2 py-0.5 rounded shrink-0">
                      Lvl {level}
                    </span>
                  </div>
                );
            ...
            <p className="text-[11px] text-zinc-500 font-semibold tracking-wide flex items-center h-full">
