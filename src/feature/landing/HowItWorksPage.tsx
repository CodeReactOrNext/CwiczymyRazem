...
const steps = [
    ...
    features: [
      {
        ...
        sub: "log session time after the fact, no timer needed",
      },
      {
        label: "Single exercises",
        sub: "exercises categorized by skill",
        count: exercisesAgregat.length,
      },
      {
        label: "Configurable exercises",
        sub: "scales, chords, strumming patterns, adjusted to your level",
      },
      {
      ...
      },
      {
        ...
        sub: "attach your own tabs to songs or plans",
      },
      {
        label: "Create your own plan",
        sub: "pick exercises, upload GP files, attach videos or photos to any step",
      },
    ],
  ...
  },
  {
    ...
    features: [
      {
        label: "Timer with category split",
        sub: "see where your time goes: technique, theory, ear training",
      },
      ...
      },
      {
        ...
        sub: "keep perfect time with an adjustable click track",
      },
      {
        label: "BPM progress tracker",
        sub: "mark BPM milestones as you push your speed further",
      },
      {
        label: "Interactive tab player",
        sub: "animated Guitar Pro tabs that scroll in sync with your session",
      },
    ],
  ...
  },
  {
    ...
    features: [
      {
        label: "Time, XP & level per category",
        sub: "technique, theory, ear training, and creativity, each tracked separately",
      },
      {
        label: "Achievements",
        sub: "unlock badges as you hit milestones: consistency, speed, repertoire",
      },
      {
        label: "Leaderboard",
        sub: "see how you rank against other guitarists globally",
      },
      {
        label: "Activity charts",
        sub: "heatmaps and graphs showing your practice patterns over time",
      },
      {
        label: "Guitar Arsenal",
        sub: "collect virtual guitars as you level up",
      },
      {
        label: "Fame Points shop",
        sub: "spend Fame Points to unlock rare guitars and cosmetics",
      },
    ],
    ...
    <m.div
      ...
      <div
        ...
        <div className={`flex flex-col ${!isEven ? "lg:order-2" : ""}`}>
          ...
          <div
            className={`text-[clamp(5rem,15vw,10rem)] font-black leading-none ${step.color} -ml-1 mb-2 select-none opacity-10`}
            aria-hidden>
            {step.number}
          </div>

          <h2 className='mb-5 whitespace-pre-line text-[clamp(1.75rem,4vw,3rem)] font-bold leading-[1.1] tracking-tight text-white'>
            {step.title}
          </h2>
...
export const HowItWorksPage = () => {
  return (
    <div className='min-h-screen overflow-x-hidden bg-zinc-950 font-sans text-zinc-100 selection:bg-cyan-500/20'>
      {/* ── Nav ── */}
      <nav className='sticky top-0 z-50 border-b border-white/5 bg-zinc-950/80 backdrop-blur-xl'>
        <div className='mx-auto flex h-16 max-w-7xl items-center justify-between px-6 lg:px-10'>
          ...
          <div className='flex items-center gap-6'>
            <Link
              href='/'
              className='hidden items-center gap-1 text-sm text-zinc-500 transition-colors hover:text-white sm:flex'>
              <ChevronRight className='h-3 w-3 rotate-180' /> Back to home
            </Link>
            <Link
              href='/signup'
              className='rounded-md bg-white px-4 py-1.5 text-sm font-bold text-zinc-950 transition-colors hover:bg-zinc-200'>
              Start Free
            </Link>
          </div>
        </div>
      ...
      {/* ── Hero ── */}
      <section className='border-b border-white/5 px-6 pb-16 pt-16 lg:px-10'>
        <div className='mx-auto max-w-7xl'>
          <m.div
            ...
            <div>
              <p className='mb-4 text-xs font-black uppercase tracking-[0.35em] text-zinc-600'>
                How it works
              </p>
              <h1 className='text-[clamp(2rem,5vw,3.75rem)] font-bold leading-[1.0] tracking-tight text-white'>
                Three steps.
                <br />
      ...
      {/* ── Final CTA ── */}
      <section className='border-t border-white/5 px-6 py-28 lg:px-10'>
        <div className='mx-auto max-w-7xl'>
          <m.div
            ...
            <div>
              <p className='mb-6 text-xs font-black uppercase tracking-[0.35em] text-zinc-600'>
                Ready?
              </p>
              <h2 className='text-[clamp(2.5rem,6vw,5rem)] font-bold leading-[0.95] tracking-tight text-white'>
                Start tracking
                <br />
            ...
            <div className='flex flex-col gap-3 sm:flex-row lg:pb-2'>
              <Link
                href='/signup'
                className='flex items-center justify-center gap-2 rounded-md bg-white px-8 py-4 text-base font-bold text-zinc-950 transition-colors hover:bg-zinc-200'>
                Try for free
                <ArrowRight className='h-4 w-4' />
              </Link>
              <Link
                href='/signup'
                className='flex items-center justify-center gap-2 rounded-md border border-white/10 px-8 py-4 text-base font-bold text-zinc-300 transition-colors hover:border-white/20 hover:text-white'>
                Start practicing
              </Link>
            </div>
          </m.div>
