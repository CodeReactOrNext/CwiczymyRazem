...
import { getScrapYield, mergeScrapParts, RARITY_SLOT_COUNT } from "./scrapYield";
...
const has = (parts: ScrapPart[], partId: string) => parts.some((p) => p.partId === partId);
...
const reachablePairs = (): Set<string> => {
  ...
  for (const e of EFFECT_DEFINITIONS) {
    collect(getScrapYield({ bom: getEffectBom(e.id, e.type), rarity: e.rarity }));
  }
  return seen;
};
...
describe("getScrapYield", () => {
  ...
  it("never yields more than four parts, whatever the rarity", () => {
    for (const rarity of ALL_RARITIES) {
      for (const g of GUITAR_DEFINITIONS) {
        expect(getScrapYield({ bom: getGuitarBom(g.id), rarity }).length).toBeLessThanOrEqual(4);
      }
    }
  });
  ...
  it("hands out more slots the rarer the gear is", () => {
    ...
    const counts = ALL_RARITIES.map((r) => getScrapYield({ bom, rarity: r }).length);
  ...
  it("falls back to a screw when a BOM was authored empty", () => {
  ...
  it("locks no part type behind rarity — a Common still gives its headline part", () => {
    ...
    expect(parts).toEqual([{ partId: "pickup", tier: "Standard", qty: 3 }]);
  });

  it("mixes tiers within one teardown instead of paying out a flat grade", () => {
    const legendary = getScrapYield({ bom: getGuitarBom(44), rarity: "Legendary" });
    expect(legendary.map((p) => p.tier)).toEqual(["Legendary", "Legendary", "Epic"]);

    const epic = getScrapYield({ bom: getGuitarBom(44), rarity: "Epic" });
    expect(epic.map((p) => p.tier)).toEqual(["Epic", "Epic", "Standard"]);
  });
  ...
  it("yields Unique parts only from Mythic gear, and only where they show", () => {
    ...
    const legendary = getScrapYield({ bom: getGuitarBom(43), rarity: "Legendary" });
  ...
  it("keeps the yield in salvage order so the headline part reads first", () => {
    ...
    expect(parts.map((p) => p.partId)).toEqual(["pickup", "neck", "body"]);
  });
});
...
describe("construction rules", () => {
  ...
  it("never yields an op-amp from a fuzz", () => {
    const parts = getScrapYield({ bom: getEffectBom(11, "Fuzz"), rarity: "Mythic" });
...
describe("tier coverage across the whole pool", () => {
  it("keeps every part reachable at every tier it can roll", () => {
    ...
    for (const def of PART_DEFINITIONS) {
      ...
      if (def.unique && !seen.has(`${def.id}:Unique`)) missing.push(`${def.id}:Unique`);
    }
...
describe("BOM coverage", () => {
  it("authors a BOM for every guitar definition", () => {
    const missing = GUITAR_DEFINITIONS.filter((g) => GUITAR_BOMS[Number(g.id)] === undefined);
    expect(missing.map((g) => g.id)).toEqual([]);
  });

  it("authors a BOM for every effect type in use", () => {
    const missing = EFFECT_DEFINITIONS.filter((e) => getEffectBom(e.id, e.type).length === 0);
  ...
  it("yields something for every guitar and effect in the pool", () => {
    for (const g of GUITAR_DEFINITIONS) {
      expect(getScrapYield({ bom: getGuitarBom(g.id), rarity: g.rarity }).length).toBeGreaterThan(0);
    }
    for (const e of EFFECT_DEFINITIONS) {
      expect(
        getScrapYield({ bom: getEffectBom(e.id, e.type), rarity: e.rarity }).length
      ).toBeGreaterThan(0);
    }
  });
