...
const RoadmapView: React.FC<RoadmapViewProps> = ({ roadmap, onUpdate }) => {
  ...
  return (
    <>
      <div className="rounded border border-zinc-800 bg-zinc-950 p-5 md:p-8">
        {/* ─── Header ─── */}
        ...
        <div className="mb-6 h-1 w-full overflow-hidden rounded bg-zinc-800">
          <div
            className="h-full rounded bg-cyan-500 transition-all duration-700"
            style={{ width: `${progress}%` }}
          />
        </div>

        {/* ─── Legend ─── */}
        <div className="mb-8 flex flex-wrap items-center gap-x-5 gap-y-1.5 rounded border border-zinc-800/60 bg-zinc-900/40 px-4 py-2.5">
          ...
          {(
            ...
            <span key={label} className="flex items-center gap-1.5 text-xs text-zinc-400">
              <span className={`h-1.5 w-1.5 rounded ${dot}`} />
              {label}
            </span>
          ))}
        ...
        <div ref={containerRef} className="relative">
          ...
          <div className="relative flex flex-col items-center" style={{ zIndex: 1 }}>
            {/* Root node */}
            <div className="max-w-sm rounded border border-zinc-600/80 bg-zinc-800/90 px-8 py-4 text-center text-sm font-bold text-zinc-100 shadow-lg shadow-black/30 ring-1 ring-zinc-600/20">
              {roadmap.goal}
            </div>
                        ...
                        {phase.steps.map((step, stepIdx) => {
                          ...
                          return (
                            <button
                              ...
                              className={`
                                flex w-full items-center gap-2.5 rounded border
                            ...
                            >
                              {isLoading ? (
                                ...
                                <span className={`h-2 w-2 shrink-0 rounded ${STATUS_DOT[status]}`} />
                              )}
                          ...
                          phase.steps.map((step, stepIdx) => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() => openDrawer(step, phase, stepIdx, phaseIdx)}
                                className={`
                                  flex max-w-[260px] items-center gap-2.5 rounded border
                                  px-3 py-2.5 text-xs font-medium transition-all
            ...
            {phases.length > 0 && (
              <div
                className={`rounded border px-8 py-4 text-center text-sm font-semibold transition-all duration-700 ${
                  progress === 100
          ...
          <>
            ...
            <div className="shrink-0 border-b border-zinc-800/80 bg-zinc-950/95 backdrop-blur-md">
              ...
              <div className="flex items-start justify-between gap-4 px-6 pt-5 pb-4">
                ...
                <button
                  onClick={() => setDrawerStepId(null)}
                  className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded text-zinc-600 transition-colors hover:bg-zinc-800 hover:text-zinc-300"
                >
                  <X className="h-4 w-4" />
                </button>
              </div>
            ...
            <div className="flex shrink-0 items-center justify-center border-b border-zinc-800/60 py-2 sm:hidden">
              <button
                onClick={() => setDrawerStepId(null)}
                className="flex items-center gap-2 rounded px-4 py-1.5 text-xs font-medium text-zinc-500 hover:bg-zinc-800 hover:text-zinc-300 active:bg-zinc-700"
              >
                ...
                <div className="flex flex-col gap-0">
                  ...
                  {(loadingExerciseIds.has(drawerInfo.step.id) || drawerInfo.step.suggestedExerciseId) && (
                    <div className="border-b border-zinc-800/60 px-6 py-5">
                      {loadingExerciseIds.has(drawerInfo.step.id) ? (
                        <div className="flex items-center gap-3 rounded border border-zinc-800 bg-zinc-900/40 px-4 py-3">
                        ...
                        return (
                          <>
                            ...
                            <button
                              ...
                              className="group relative flex w-full items-center gap-4 overflow-hidden rounded border border-cyan-800/40 bg-cyan-950/30 px-4 py-4 text-left transition-all hover:border-cyan-600/50 hover:bg-cyan-950/50"
                            >
                              {/* glow */}
                              <div className="pointer-events-none absolute inset-0 rounded bg-gradient-to-r from-cyan-500/5 via-transparent to-transparent" />
                              <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded bg-cyan-500/15 ring-1 ring-cyan-500/30 transition group-hover:bg-cyan-500/25 group-hover:ring-cyan-500/50">
                  ...
                  <div className="border-b border-zinc-800/60 px-6 py-5">
                    <div className="space-y-3 text-sm leading-relaxed text-zinc-400">
                      {drawerInfo.step.description.split("\n").map((line, i) => {
                        ...
                        if (sectionMatch) {
                          return (
                            <p key={i} className="mt-4 text-[10px] font-semibold capitalize tracking-widest text-zinc-600 first:mt-0">
                              {sectionMatch[1]}
                            </p>
                          );
                        }
                        if (line.startsWith("- ")) {
                          return (
                            <div key={i} className="flex items-start gap-2.5">
                              <span className="mt-[7px] h-1.5 w-1.5 shrink-0 rounded bg-zinc-700" />
                  ...
                  {drawerInfo.step.successCriteria && (
                    <div className="border-b border-zinc-800/60 px-6 py-5">
                      ...
                      <div className="rounded border border-cyan-900/50 bg-cyan-950/25 px-4 py-3.5">
                      ...
                      {loadingLessonsId === drawerInfo.step.id ? (
                        <div className="space-y-2">
                          {[0, 1, 2].map((i) => (
                            <div
                              key={i}
                              className="flex h-[61px] animate-pulse items-center gap-3 rounded border border-zinc-800 bg-zinc-900/60 px-3"
                            >
